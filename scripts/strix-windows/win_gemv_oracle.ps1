...
cargo build --release -p spark-model --no-default-features --features cuda,gpu-examples --example dense_gemv_bf16_oracle 2>&1 | Select-Object -Last 3
if ($LASTEXITCODE -ne 0) { "BUILD FAILED: $LASTEXITCODE"; exit 1 }
...
& .\target\release\examples\dense_gemv_bf16_oracle.exe *> C:\Users\azeez\q38-win-gemv-oracle.log
"ORACLE-EXIT: $LASTEXITCODE"
Get-Content C:\Users\azeez\q38-win-gemv-oracle.log -Tail 14
