...
# Destructive requant path (stable on Windows; the preservation BF16 decode
# paths crash under load — see the handoff bisection note).
$ErrorActionPreference = "Continue"
$Repo = "C:\Users\azeez\code\atlas-inf-pr9"
$Bin = "$Repo\target\x86_64-pc-windows-msvc\release\spark.exe"
$Log = "C:\Users\azeez\q38-win-st995.log"
...
"server up - running PINNED ST-995 (no param overrides)"
& $Bin benchmark run bfcl-subset --url http://127.0.0.1:8081 --model unsloth/Qwen3.8-27B-NVFP4 2>&1 | ForEach-Object { "$_" } | Out-File C:\Users\azeez\q38-win-st995-bench.log -Encoding utf8
"ST995 EXIT: $LASTEXITCODE"
Get-Content C:\Users\azeez\q38-win-st995-bench.log -Tail 45
Get-Process spark -ErrorAction SilentlyContinue | Stop-Process -Force
"WIN ST995 DONE"
