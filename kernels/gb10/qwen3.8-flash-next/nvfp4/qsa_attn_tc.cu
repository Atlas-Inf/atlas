...
// QSA prefill attention on TENSOR CORES.
//
...
#define ATLAS_PREFILL_ENTRY qsa_prefill_attn_tc
#endif
...
__device__ __forceinline__ unsigned long long qsa_tc_key_off(
...
extern "C" __global__ void ATLAS_PREFILL_ENTRY(
    ...
    const unsigned int kv_head = blockIdx.x;      // one CTA per (row, kv head)
    const unsigned int r = blockIdx.y;
    const unsigned int gqa = num_q_heads / num_kv_heads;
    const unsigned int qh0 = kv_head * gqa;
    ...
    #define QSA_TC_KEY_OFF(t) qsa_tc_key_off((t), topk_ratio, ratio, ratio_p2, \
        ratio_sh, ratio_mask, complete, block_size, bs_p2, bs_sh, bs_mask, \
        my_list, block_table, page_stride, row_elems, kv_head, head_dim)

    const unsigned int tid = threadIdx.x;
    const unsigned int warp_id = tid / 32;
    const unsigned int lane_id = tid % 32;

    if (kv_head >= num_kv_heads || gqa > BR) return;

    // Rows of the M tile are this row's q-heads: 0..gqa-1 live, the rest zero.
    const unsigned int q_len = gqa;
    ...
    // Saves one commit/wait/sync vs separate Q and K loads.
    {
        ...
        for (unsigned int idx = tid; idx < TILE_CHUNKS; idx += 128) {
            ...
            if (row < q_len) {
                const void* gmem = (const void*)&Q_batch[(qh0 + row) * head_dim + col];
        ...
        if (num_kv_blocks > 0) {
            for (unsigned int idx = tid; idx < TILE_CHUNKS_KV_TILE; idx += 128) {
                ...
                if (k_row < seq_len) {
                    const void* gmem = (const void*)&K_batch[QSA_TC_KEY_OFF(k_row) + col];
                    asm volatile("cp.async.cg.shared.global [%0], [%1], 16;" :: "r"(smem_addr), "l"(gmem));
                } else {
    ...
    for (unsigned int kv_block = kv_block_lo; kv_block < num_kv_blocks; kv_block++) {
        ...
        // === Start async V load into smem_V (overlaps with QK^T below) ===
        {
            ...
            for (unsigned int idx = tid; idx < TILE_CHUNKS_KV_TILE; idx += 128) {
                ...
                if (v_row < seq_len) {
                    const void* gmem = (const void*)&V_batch[QSA_TC_KEY_OFF(v_row) + col];
                    asm volatile("cp.async.cg.shared.global [%0], [%1], 16;" :: "r"(smem_addr), "l"(gmem));
                } else {
    ...
    // === Final normalization and store ===
    {
        ...
        for (int nt = 0; nt < N_TILES_PER_WARP; nt++) {
            ...
            if (row0 < q_len && col0 < head_dim) {
                ...
                *(unsigned int*)&o_base[(qh0 + row0) * head_dim + col0] = lo | (hi << 16);
            }
            if (row1 < q_len && col0 < head_dim) {
                ...
                *(unsigned int*)&o_base[(qh0 + row1) * head_dim + col0] = lo | (hi << 16);
            }
        }
    }
}
