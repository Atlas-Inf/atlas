...
// QSA prefill attention on TENSOR CORES -- both kv heads, BC=32.
//
// v3 of qsa_attn_tc. tc2 packs both kv heads (M waste 1.33x) but pays BC=16
// for the second K/V buffer, and BC=16 nearly halves the achieved rate:
// tc1 (BC=32) ran 48.9 TFLOP in 3.43 s = 14.3 TFLOP/s, tc2 24.3 TFLOP in
// 2.91 s = 8.4. tc2 wins only by doing less work. This keeps tc2's packing
// AND tc1's tile by SINGLE-buffering K.
//
// Why single-buffer: at BC=32 with two kv heads a double-buffered K is
// 67,584 B and the total 121,088 B, which the driver REJECTS -- static
// shared per block caps at 101,376 B (this file's own Gemma-4 note says so,
// and the JIT confirms it with CUDA_ERROR_INVALID_PTX). Single-buffered K is
// 33,792 B and the total 87,296 B, which fits.
//
...
#define ATLAS_PREFILL_ENTRY qsa_prefill_attn_tc3
#endif
...
__device__ __forceinline__ unsigned long long qsa_tc3_key_off(
...
extern "C" __global__ void ATLAS_PREFILL_ENTRY(
    ...
    #define QSA_TC3_KEY_OFF(t, kvh_) qsa_tc3_key_off((t), topk_ratio, ratio, ratio_p2, \
    ...
    __shared__ __nv_bfloat16 smem_K[2][BC][HDIM_PAD];  // [kv head] -- SINGLE buffered
    ...
    for (unsigned int kv_block = kv_block_lo; kv_block < num_kv_blocks; kv_block++) {
        ...
        // === K[kv_block] into the single buffer ===
        // The double buffer is gone because a second one costs 33,792 B and the
        // driver caps a block's static shared at 101,376 B -- at BC=32 with two
        // kv heads, keeping it JIT-fails with CUDA_ERROR_INVALID_PTX. The
        // `__syncthreads()` at the bottom of the previous iteration is what
        // makes overwriting this buffer safe: every warp has finished reading
        // smem_K in its QK pass by then. K/compute overlap is the price; the V
        // load below still overlaps QK.
        {
            const unsigned int chunks_per_row_k0 = HDIM / 8;
            for (unsigned int idx = tid; idx < 2u * TILE_CHUNKS_KV_TILE; idx += 128) {
                ...
                unsigned int row = j / chunks_per_row_k0;
                unsigned int chunk = j % chunks_per_row_k0;
                unsigned int col = chunk * 8;
                unsigned int k_row = kv_start + row;
                unsigned int smem_addr = __cvta_generic_to_shared(&smem_K[kvh][row][col]);

                if (k_row < seq_len) {
                    const void* gmem = (const void*)&K_batch[QSA_TC3_KEY_OFF(k_row, kvh) + col];
                    asm volatile("cp.async.cg.shared.global [%0], [%1], 16;" :: "r"(smem_addr), "l"(gmem));
                } else {
                    *((uint4*)&smem_K[kvh][row][col]) = make_uint4(0, 0, 0, 0);
                }
            }
            asm volatile("cp.async.commit_group;");
            asm volatile("cp.async.wait_group 0;");
        ...
        // === Start async V load into smem_V (overlaps with QK^T below) ===
        {
            ...
            for (unsigned int idx = tid; idx < 2u * TILE_CHUNKS_KV_TILE; idx += 128) {
                ...
                if (v_row < seq_len) {
                    const void* gmem = (const void*)&V_batch[QSA_TC3_KEY_OFF(v_row, kvh) + col];
                    asm volatile("cp.async.cg.shared.global [%0], [%1], 16;" :: "r"(smem_addr), "l"(gmem));
                } else {
        ...
        if (warp_id < 2) {
            ...
            for (unsigned int ks = 0; ks < (HDIM / 16); ks++) {
                ...
                const unsigned short* sK_u16 = (const unsigned short*)smem_K[warp_id & 1];
                #pragma unroll
