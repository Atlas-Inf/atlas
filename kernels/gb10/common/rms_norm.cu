...
// Sigmoid-gated variant: out = rms_norm(x) * sigmoid(gate).
//
// Qwen3.5/3.6 gate with SiLU; qwen4_exp declares `output_gate_type =
// "sigmoid"` and gates with a plain sigmoid. That is a correctness switch, not
// a tuning knob -- the two differ most exactly where the gate is doing its job.
//
// Duplicated rather than templated or branched, deliberately. `gated_rms_norm`
// above must stay byte-reproducible: the fused GDN verify kernels
// (gdn_verify_fused_k2.cu) mirror its accumulation order instruction-for-
// instruction under `--fmad=false`, and a template or an extra parameter risks
// different codegen for the path that already ships. The only difference below
// is the four gate lines.
extern "C" __global__ void gated_rms_norm_sigmoid(
    ...
    for (unsigned int i = tid; i < quad_size; i += blockDim.x) {
        ...
        float s0 = 1.0f / (1.0f + expf(-g0));  // sigmoid
        float s1 = 1.0f / (1.0f + expf(-g1));  // sigmoid
        float s2 = 1.0f / (1.0f + expf(-g2));  // sigmoid
        float s3 = 1.0f / (1.0f + expf(-g3));  // sigmoid
