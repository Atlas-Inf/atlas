...
extern "C" __global__ void w4a16_relu2_gemv_batch32(
    ...
    for (unsigned int base = 0; base < M; base += SHARED_BATCH_M) {
        const unsigned int batch = M - base < SHARED_BATCH_M ? M - base : SHARED_BATCH_M;
        for (unsigned int m = 0; m < batch; m++) {
            const __nv_bfloat16* src = A + (unsigned long long)(base + m) * K;
            __nv_bfloat16* dst = s_A + (unsigned long long)m * K;
            for (unsigned int i = threadIdx.x; i < K; i += BLOCK_SIZE) dst[i] = src[i];
        }
        __syncthreads();
        ...
        for (unsigned int t = 0; t < N_PER_WARP; t++) {
            const unsigned int n = n_base + warp * N_PER_WARP + t;
            if (n < N) {
                float acc[SHARED_BATCH_M];
                #pragma unroll
                for (int m = 0; m < SHARED_BATCH_M; m++) acc[m] = 0.0f;
                ...
                for (unsigned int k8 = lane; k8 < K8; k8 += WARP_SIZE) {
                    ...
                    unsigned char scale_byte =
                        B_scale[(unsigned long long)n * num_groups + base_k / GROUP_SIZE];
#if defined(__SCALE__) || defined(__HIP_PLATFORM_AMD__)
                    float scale = scl_fp8_grp(scale_byte) * scale2;
#else
                    __nv_fp8_e4m3 fp8;
                    *(unsigned char*)&fp8 = scale_byte;
                    ...
                    for (unsigned int m = 0; m < batch; m++) {
                        ...
                        for (int b = 0; b < 4; b++) {
                            __nv_bfloat16 a_lo, a_hi;
                            *(unsigned short*)&a_lo = (unsigned short)(ar[b] & 0xFFFF);
                            *(unsigned short*)&a_hi = (unsigned short)(ar[b] >> 16);
                            float al = fmaxf(__bfloat162float(a_lo), 0.0f); al *= al;
                            float ah = fmaxf(__bfloat162float(a_hi), 0.0f); ah *= ah;
                            acc[m] += al * w_lo[b] + ah * w_hi[b];
                        }
                    }
                }
                ...
                for (int off = WARP_SIZE / 2; off > 0; off >>= 1) {
                    #pragma unroll
                    for (int m = 0; m < SHARED_BATCH_M; m++)
                        acc[m] += __shfl_down_sync(0xFFFFFFFF, acc[m], off);
                }
                if (lane == 0) {
                    for (unsigned int m = 0; m < batch; m++)
                        C[(unsigned long long)(base + m) * N + n] = __float2bfloat16(acc[m]);
                }
            }
        }
        __syncthreads();
