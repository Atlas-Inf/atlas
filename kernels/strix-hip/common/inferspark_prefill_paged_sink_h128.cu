...
// HDIM=128 build: DFlash2 drafters use head_dim 128; the HDIM=256 build
// reads dims 128..255 of each row — the neighbouring head's data — so the
// sink kernels must be compiled at the drafter's real head_dim.
// DFlash/DSpark paged attention with per-head attention sinks.
// Same as inferspark_prefill_paged.cu plus ATLAS_ATTN_SINKS (last extra arg).
...
#define ATLAS_ATTN_SINKS
#define HDIM 128
#define KERNEL_NAME inferspark_prefill_paged_sink_h128
#define K_CACHE_TYPE const __nv_bfloat16* __restrict__
#define V_CACHE_TYPE const __nv_bfloat16* __restrict__
#define KERNEL_EXTRA_PARAMS , const float inv_sqrt_d, const __nv_bfloat16* __restrict__ sinks
