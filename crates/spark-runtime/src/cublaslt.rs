...
pub fn bf16_gemm_act_weight_t(
    ...
    let ctx = ctx()?;
    // Descriptor, layouts and heuristic pick are cached per shape
    // (`bf16_plan.rs`); only the data pointers and the stream vary per call.
    bf16_plan::with_plan(ctx.handle, ctx.ws_size, m, n, k, CUBLAS_OP_T, |plan| {
        let alpha: f32 = 1.0;
        let beta: f32 = 0.0;
        let status = unsafe {
            cublasLtMatmul(
