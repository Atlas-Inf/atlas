...
pub(crate) fn parse_qwen4_exp(raw: &Value) -> Result<ModelConfig> {
    ...
    // The raw string is kept too: `qwen3_ssm::init` selects the gated-norm
    // kernel from it and REFUSES anything that is neither "silu" nor
    // "sigmoid", so an unknown activation is a malformed config rather than a
    // silent fall back to the family default.
    config.output_gate_type = text
        .get("output_gate_type")
        .and_then(Value::as_str)
        .unwrap_or_default()
        .to_string();
    ...
    // Every other nested-config arm ends here, and this one was returning
    // early: `finalize_config` parses `quantization_config` off the TOP level
    // (ModelOpt writes it beside `text_config`, so serde's derive on
    // text_config alone never sees it) and then runs the shared
    // `validate_config` SSM/head-count checks. Without it an NVFP4 checkpoint
...
fn parse_hyper_connections(text: &Value, config: &mut ModelConfig) -> Result<()> {
    ...
    // `hc_count` is ALSO kept under its own name. `hc_mult` is what the
    // engine dispatches on (it is DeepSeek-V4's plumbing, shared); `hc_count`
    // is what `weight_manifest::qwen4_exp` and the CPU reference read, so the
    // manifest describes the checkpoint in the checkpoint's own vocabulary and
    // cannot drift from the engine's mapping without a test noticing.
    config.hc_count = config.hc_mult;
...
fn parse_indexer(text: &Value, config: &mut ModelConfig) -> Result<()> {
    ...
    // The `indexer_*` names are kept alongside the `index_*` mapping for the
    // same reason as `hc_count`: the manifest and the reference read the
    // checkpoint's own field names. `indexer_kv_heads` has no `index_*`
    // counterpart at all -- DeepSeek-V4's indexer has no separate KV head
    // count -- so dropping it would lose a fact the checkpoint states.
    config.indexer_n_heads = config.index_n_heads;
    config.indexer_head_dim = config.index_head_dim;
    config.indexer_budget = config.index_topk;
    config.indexer_compress_ratio = ratio;
    config.indexer_kv_heads = g("indexer_kv_heads");
    Ok(())
}
...
fn parse_ngram_ple(text: &Value, config: &mut ModelConfig) -> Result<()> {
    ...
    config.ngram_vocab_size_base = text
        .get("ngram_vocab_size_base")
        .and_then(Value::as_u64)
        .unwrap_or(0);
    config.split_ngram_parts = g("split_ngram_parts");
    ...
    // ...and again under the checkpoint's own names, which is what
    // `Qwen4ExpNgram` (config/ngram_qwen4exp.rs) reads. That type DERIVES the
    // multipliers, per-head primes and offsets from the config and asserts
    // they equal the buffers the checkpoint ships, so keeping both spellings
    // is what lets the derivation and the shipped values check each other.
    config.ngram_size = ngram_size;
    config.heads_per_ngram = heads_per_ngram;
    config.ple_embed_dim = text
        .get("ple_embed_dim")
        .and_then(Value::as_u64)
        .map_or(config.hidden_size, |v| v as usize);
    ...
    // ONE-INDEXED, as HF writes them: the reference selects with
    // `ple_layer_ids.index(layer_idx + 1)`, so `[2]` is decoder layer 1 and
    // the weights live at `layers.1.ple.*`. A 0 here is therefore malformed,
    // not "layer 0" -- checking `l < num_layers` alone would accept it and
    // then look for a tower one layer earlier than where it is.
    for &l in &config.ple_layer_ids {
        ensure!(
            (1..=config.num_hidden_layers).contains(&l),
            "qwen4_exp ple_layer_ids are ONE-indexed decoder layers in \
             [1, {}], got {l}",
            config.num_hidden_layers,
        );
    }
    ...
    if config.ngram_vocab_size_base > 0 {
        let align = text
            ...
            .and_then(Value::as_u64)
            .unwrap_or(NGRAM_VOCAB_ALIGN);
        config.make_ngram_vocab_size_divisible_by = align;
        ensure!(
