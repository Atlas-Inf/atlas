...
fn main() -> Result<()> {
    ...
    let n = args.get(1).map_or(Ok(DEFAULT_N), |value| value.parse())?;
    let k = args.get(2).map_or(Ok(DEFAULT_K), |value| value.parse())?;
    let repeats = args.get(3).map_or(Ok(1usize), |value| value.parse())?;
    ensure!(n.is_multiple_of(FP8_BLOCK));
    ensure!(k.is_multiple_of(FP8_BLOCK));
    ensure!(repeats > 0);
