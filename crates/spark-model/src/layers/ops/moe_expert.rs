...
/// Coarse-N expert GEMV. A staged in smem. Grid (ceil(N/32), top_k, tokens).
pub fn moe_expert_gemv_wide(
    gpu: &dyn GpuBackend,
    kernel: KernelHandle,
    input: DevicePtr,
    packed_ptrs: DevicePtr,
    ...
    KernelLaunch::new(gpu, kernel)
        .grid([div_ceil(n, 32), top_k, num_tokens.max(1)])
        .block([256, 1, 1])
        .shared_mem(64 + k * 2)
        .arg_ptr(input)
        .arg_ptr(packed_ptrs)
...
pub fn moe_expert_gemv_wide_grouped(
    ...
    KernelLaunch::new(gpu, kernel)
        ...
        .block([256, 1, 1])
        .shared_mem(64 + 4 * k * if relu2_input { 4 } else { 2 })
        .arg_ptr(input)
        .arg_ptr(packed_ptrs)
        .arg_ptr(scale_ptrs)
        .arg_ptr(scale2_vals)
